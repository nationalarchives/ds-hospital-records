...
from app.hospitaldetails.models import (
    ...
    Post1948Type,
    Post1974County,
    Post1982DistrictAuthority,
    Post1982RegionalAuthority,
    Post1996County,
    Pre1948Status,
    Pre1948Type,
    Pre1974County,
    Pre1982DistrictAuthority,
    Pre1982RegionalAuthority,
    RecordsInfo,
    RegionalBoard,
    Repository,
)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--user", type=str, default=os.getenv("MSSQL_USER"), help="MSSQL username"
        )
        parser.add_argument(
            "--password",
            type=str,
    ...
    def get_mssql_connection(self, host, port, database, user, password):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Connected to MSSQL database: {database}")
            )
            return conn
    ...
    def clear_data(self):
        ...
        for name, model in models_to_clear:
            ...
            self.stdout.write(
                self.style.WARNING(f"Deleted {count} records from {name}")
            )
    ...
    def set_foreign_key_if_valid(
        self, obj, field_name, fk_id, model_class, error_label
    ):
        ...
        if fk_id and fk_id != 1:
            if model_class.objects.filter(id=fk_id).exists():
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"{error_label} {fk_id} not found, setting to None"
                    )
                )
        ...
        for row in rows:
            try:
                ...
                hospital_data = {
                    ...
                    "foundation_year_approximate": bool(
                        row.get("FoundationDateApproximate", False)
                    ),
                    "closed": bool(row.get("Closed", False)),
                    "closure_date": row.get("ClosureDate"),
                    "closure_year_approximate": bool(
                        row.get("ClosureDateApproximate", False)
                    ),
                ...
                hospital_data.update(
                    self.prepare_timestamps(
                        row.get("InputDate"), row.get("LastUpdatedDate")
                    )
                )
                ...
                fk_data = {
                    ...
                    "pre_1982_regional_authority_id": row.get(
                        "RegionalAuthority1974ID"
                    ),
                    "post_1982_regional_authority_id": row.get(
                        "RegionalAuthority1982ID"
                    ),
                    "pre_1982_district_authority_id": row.get(
                        "DistrictAuthority1974ID"
                    ),
                    "post_1982_district_authority_id": row.get(
                        "DistrictAuthority1982ID"
                    ),
                }
                ...
                if not dry_run:
                    ...
                    hospital, created = Hospital.objects.update_or_create(
                        id=hospital_id, defaults=hospital_data
                    )
                    ...
                    self.add_m2m_if_true(
                        hospital,
                        hospital.pre_1948_status,
                        row.get("Pre1948Voluntary"),
                        Pre1948Status,
                        "Voluntary",
                    )
                    self.add_m2m_if_true(
                    ...
                    self.add_m2m_if_true(
                        hospital,
                        hospital.pre_1948_type,
                        row.get("Pre1948General"),
                        Pre1948Type,
                        "General",
                    )
                    self.add_m2m_if_true(
                    ...
                    self.add_m2m_if_true(
                        hospital,
                        hospital.post_1948_type,
                        row.get("Post1948Acute"),
                        Post1948Type,
                        "Acute",
                    )
                    self.add_m2m_if_true(
                    ...
                    self.add_m2m_if_true(
                        hospital,
                        hospital.post_1948_type,
                        row.get("Post1948OtherType"),
                        Post1948Type,
                        "Other",
                    )

                    # Handle county foreign key relationships using helper method
                    self.set_foreign_key_if_valid(
                        hospital,
                        "pre_1974_county_id",
                        fk_data.get("pre_1974_county_id"),
                        Pre1974County,
                        "Pre-1974 County",
                    )
                    self.set_foreign_key_if_valid(
                        hospital,
                        "post_1974_county_id",
                        fk_data.get("post_1974_county_id"),
                        Post1974County,
                        "Post-1974 County",
                    )
                    self.set_foreign_key_if_valid(
                        hospital,
                        "post_1996_county_id",
                        fk_data.get("post_1996_county_id"),
                        Post1996County,
                        "Post-1996 County",
                    )
                    ...
                    self.set_foreign_key_if_valid(
                        ...
                        fk_data.get("management_committee_id"),
                        ManagementCommittee,
                        "Management Committee",
                    )
                    self.set_foreign_key_if_valid(
                        hospital,
                        "pre_1982_regional_authority_id",
                        fk_data.get("pre_1982_regional_authority_id"),
                        Pre1982RegionalAuthority,
                        "Pre-1982 Regional Authority",
                    )
                    self.set_foreign_key_if_valid(
                        hospital,
                        "post_1982_regional_authority_id",
                        fk_data.get("post_1982_regional_authority_id"),
                        Post1982RegionalAuthority,
                        "Post-1982 Regional Authority",
                    )
                    self.set_foreign_key_if_valid(
                        hospital,
                        "pre_1982_district_authority_id",
                        fk_data.get("pre_1982_district_authority_id"),
                        Pre1982DistrictAuthority,
                        "Pre-1982 District Authority",
                    )
                    self.set_foreign_key_if_valid(
                        hospital,
                        "post_1982_district_authority_id",
                        fk_data.get("post_1982_district_authority_id"),
                        Post1982DistrictAuthority,
                        "Post-1982 District Authority",
                    )

                    hospital.save()

                    # Update created_at and last_updated_at with original values using helper
                    update_fields = self.prepare_timestamps(
                        row.get("InputDate"), row.get("LastUpdatedDate")
                    )
                    if update_fields:
            ...
            try:
                ...
                if not dry_run:
                    ...
                    repository, created = Repository.objects.update_or_create(
                        id=repository_id, defaults=repository_data
                    )
                    ...
                    if update_fields:
                        Repository.objects.filter(id=repository.id).update(
                            **update_fields
                        )
        ...
        for row in rows:
            try:
                ...
                if not Hospital.objects.filter(id=hospital_id).exists():
                    self.stdout.write(
                        self.style.WARNING(
                            f"Hospital {hospital_id} not found, skipping folder {row.get('Folder_No')}"
                        )
                    )
                    skipped_count += 1
                    continue
                ...
                if not Repository.objects.filter(id=repository_id).exists():
                    self.stdout.write(
                        self.style.WARNING(
                            f"Repository {repository_id} not found, skipping folder {row.get('Folder_No')}"
                        )
                    )
                    skipped_count += 1
                    continue
                ...
                if not dry_run:
                    ...
                    records_info, created = RecordsInfo.objects.update_or_create(
                        id=folder_id, defaults=records_data
                    )
                    ...
                    self.add_m2m_if_true(
                        records_info,
                        records_info.finding_aids,
                        row.get("BriefGuideAids"),
                        FindingAids,
                        "Brief guide (BG)",
                    )
                    self.add_m2m_if_true(
                    ...
                    if update_fields:
                        RecordsInfo.objects.filter(id=records_info.id).update(
                            **update_fields
                        )
